    ...
    ILogger<ApiKeyAuthenticationMiddleware> logger)
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        {
            logger.LogCritical(
                "API authentication is not configured. Set Authentication__ApiKey.");
            await WriteError(
                context,
        ...
        if (providedKeys.Count != 1 ||
            string.IsNullOrWhiteSpace(providedKeys[0]) ||
            !KeysMatch(configuredKey, providedKeys[0]!))
        {
            logger.LogWarning(
                "Rejected unauthenticated API request from {RemoteAddress}.",
